...
describe('Appointment Controller', () => {
  describe('POST /api/appointments', () => {
    it('should create appointment successfully', async () => {
      ...
      const appointmentDate = new Date(Date.now() + (daysToAdd + 1) * 24 * 60 * 60 * 1000);
      appointmentDate.setHours(10, 0, 0, 0); // Set to 10 AM

      const appointmentData = {
        date: appointmentDate.toISOString(),
        description: 'This is a test appointment for psychological consultation',
      };
      ...
      expect(response.body.data.appointment.description).toBe(appointmentData.description);
  ...
  describe('GET /api/appointments/:id', () => {
    let appointment: InstanceType<typeof Appointment>;
    ...
    it('should get appointment by id successfully', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.appointment.id).toBe(appointment._id.toString());
      expect(response.body.data.appointment.description).toBe(appointment.description);
  ...
  describe('PUT /api/appointments/:id', () => {
    ...
    it('should update appointment successfully', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.appointment.description).toBe(updateData.description);
    ...
    it('should update appointment date', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(new Date(response.body.data.appointment.date).toISOString()).toBe(newDate);
    });
  ...
  describe('DELETE /api/appointments/:id', () => {
    let futureAppointment: InstanceType<typeof Appointment>;
    let anotherFutureAppointment: InstanceType<typeof Appointment>;
    ...
    it('should delete future appointment successfully', async () => {
      ...
      const deletedAppointment = await Appointment.findById(futureAppointment._id);
    ...
    it('should fail to delete past active appointment', async () => {
      ...
      )) as InstanceType<typeof Appointment>;
