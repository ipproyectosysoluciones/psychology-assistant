...
export const getUserProfile = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const user = await User.findById(req.user!._id).select('-password -twoFASecret');

  if (!user) {
    throw AppError.notFound('User not found');
  }

  logger.info({ userId: req.user!._id }, 'User profile retrieved');
...
export const updateUserProfile = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    // Validate input
    if (!name && !email) {
      throw AppError.badRequest('At least one field (name or email) must be provided');
    }
...
export const changePassword = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const passwordValidation = validationService.validatePasswordStrength(newPassword);
  if (!passwordValidation.valid) {
    ...
    throw AppError.badRequest(`New password is too weak: ${passwordValidation.error}`);
  }
  ...
  auditLog(user._id, AUDIT_EVENTS.PASSWORD_CHANGE, 'User', { userId: user._id }, 'SUCCESS');

  logger.info({ userId: req.user!._id }, 'Password changed successfully');

  const response = ApiResponse.success({ passwordChanged: true }, 'Password changed successfully');
...
export const deactivateAccount = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    ...
    logger.info({ userId: req.user!._id }, 'Account deactivated');
...
export const getUserStats = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const totalAppointments = await user.model('Appointment').countDocuments({ user: req.user!._id });
  ...
  const totalSessions = await user.model('Session').countDocuments({ user: req.user!._id });
  ...
  logger.info({ userId: req.user!._id }, 'User stats retrieved');

  const response = ApiResponse.success({ stats }, 'User statistics retrieved successfully');
 ...
 */
export const deleteAllUserData = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    const { password } = req.body;
