...
router.post(
  ...
  protect,
  strictLimiter,
  [body('password').trim().notEmpty().withMessage('Password is required to deactivate account')],
  validateRequest,
  deactivateAccount
);
...
router.delete(
  ...
  protect,
  strictLimiter,
  [body('password').trim().notEmpty().withMessage('Password is required to delete user data')],
  validateRequest,
  deleteAllUserData
);
